 ...
 *  - **partial** {Boolean}, partial have the sharded system return a partial result from mongos.
 *
 * @param {Object} options the new query options
 * @param {Function} [callback] this optional callback will be called after executing this method. The first parameter will contain an error object when the batchSize given is not a valid number or when the cursor is already closed while the second parameter will contain a reference to this object upon successful execution.
 * @return {Cursor} an instance of this object.
 * @api public
 */
Cursor.prototype.queryOptions = function(options) {
  if(this.state != Cursor.INIT) throw new Error("Query options for a cursor cannot be changed after it was opened");

  // Overwrite options
  this.hint = options.hint || this.hint;
  this.snapshot = options.snapshot || this.snapshot;
  this.timeout = options.timeout || this.timeout;
  this.tailable = options.tailable || this.tailable;
  this.awaitdata = options.awaitdata || this.awaitdata;
  this.batchSize = options.batchSize || this.batchSize;
  this.raw = options.raw || this.raw;
  this.returnKey = options.returnKey || this.returnKey;
  this.maxScan = options.maxScan || this.maxScan;
  this.min = options.min || this.min;
  this.max = options.max || this.max;
  this.showDiskLoc = options.showDiskLoc || this.showDiskLoc;
  this.comment = options.comment || this.comment;
  this.tailableRetryInterval = options.tailableRetryInterval || this.tailableRetryInterval;
  this.exhaust = options.exhaust || this.exhaust;
  this.partial = options.partial || this.partial;
}
