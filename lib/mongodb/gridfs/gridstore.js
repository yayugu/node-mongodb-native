 ...
 *  - **chunk_size** {Number}, size for the chunk. Defaults to **{GridStore.DEFAULT_CHUNK_SIZE}**.
...
var GridStore = function GridStore(db, id, filename, mode, options) {
  ...
  this.internalChunkSize = this.options['chunkSize'] == null ? GridStore.DEFAULT_CHUNK_SIZE : this.options['chunkSize'];
}
      ...
      collection.find(query, {readPreference:self.readPreference}, function(err, cursor) {
        ...
        cursor.nextObject(function(err, doc) {
          ...
          if(doc != null) {
            ...
            self.internalChunkSize = self.internalChunkSize == null ? GridStore.DEFAULT_CHUNK_SIZE : self.internalChunkSize;
            self.length = 0;
          } else {
            self.length = 0;
 ...
 * @constant
 */
GridStore.DEFAULT_CHUNK_SIZE = 1024 * 256;
