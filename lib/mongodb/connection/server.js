...
Server.prototype.connect = function(dbInstance, options, callback) {
  ...
  var connectCallback = function(_server) {
    return function(err, reply) {  
      ...
      // Let's execute the buildInfo command
      var db_command = DbCommand.createDbCommand(dbInstance, {buildInfo:true});
      ...
      server._registerHandler(db_command, false, connection, function(err, result) {
        if(err) return internalCallback(err, null);

        // Extract build info
        var buildInfo = result.documents[0];
        // Set server capabilities
        server.serverCapabilities = new ServerCapabilities(buildInfo);
