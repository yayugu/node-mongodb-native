...
Collection.prototype.findOneAndDelete = function(filter, options, callback) {
  ...
  if(options.w) commandOptions.w = options.w;
  if(options.wtimeout) commandOptions.wtimeout = options.wtimeout;
  if(options.j) commandOptions.j = options.j;
...
Collection.prototype.findAndModify = function(query, sort, doc, options, callback) {
  ...
  // If we have a w set larger than 1 or string we will pipeline the operation
  if(typeof options.w == 'string' || options.w > 1) {
    // Build last error command
    var lastErrorObject = {
      getLastError: 1, w: options.w
    }

    // Decorate the command
    if(options.j) lastErrorObject.j = options.j;
    if(options.wtimeout) lastErrorObject.wtimeout = options.wtimeout;

    // Prepare the pipelined operations
    var queries = [queryObject, lastErrorObject];

    // Execute the pipelined operations against mongodb
    this.s.db.command(queries, options, function(err, results) {
      if(err) return handleCallback(callback, err, null);
      return handleCallback(callback, null, results[0]);
    });
  } else {
    // Execute the command
